...
// GET /api/webull/quote?symbol=AAPL
// Precio y variación % EN VIVO de un solo ticker — usado en el encabezado
// del chart page (a diferencia de los bars diarios, que no se actualizan intradía).
export async function GET(request: NextRequest) {
  try {
    ...
    if (!snap) {
      return NextResponse.json({ success: false, symbol, error: "Sin snapshot para este símbolo" }, { status: 502 });
    }

    const price = parseFloat(snap.price) > 0 ? parseFloat(snap.price) : parseFloat(snap.pre_close) || null;
    const changeRatio = parseFloat(snap.change_ratio);
    const change = !isNaN(changeRatio) ? changeRatio * 100 : null;

    return NextResponse.json({ success: true, symbol, price, change });
  } catch (err: any) {
